...
/// - `schoolx://repo|project|pr|issue?…` — emits `deep-link-entity` to the frontend
///
...
mod product_identity_tests {
    use super::{classify_deep_link, DeepLinkRejection};
    ...
    fn product_scheme_is_accepted_for_every_deep_link_kind() {
        for kind in ["connect", "join", "message", "add-community", "nostr-bind"] {
            ...
            let parsed = classify_deep_link(&url)
                .unwrap_or_else(|error| panic!("{kind} must be accepted, got {error:?}"));
            assert_eq!(parsed.host_str(), Some(kind));
        }
    }

    /// Legacy `buzz://` links must not be acted on at the OS boundary, for
    /// every kind — not just the ones a test happened to cover.
    #[test]
