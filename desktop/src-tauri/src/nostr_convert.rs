...
//! testable with hand-crafted events (see `nostr_convert_tests.rs`).
...
pub fn channel_detail_from_event(event: &Event) -> Result<ChannelDetailInfo, String> {
    ...
    Ok(ChannelDetailInfo {
        ...
        purpose_set_by: None,
        purpose_set_at: None,
        // The channel's creator, from the relay-written `created_by` tag —
        // **not** `event.pubkey`, which is the relay's own key because the
        // relay signs kind:39000. Empty means the relay did not say (an event
        // written before the tag existed); callers must read that as "cannot
        // verify" and refuse, never substitute a default. SchoolX catalog
        // adoption anchors channel ownership on this value
        // (`CATALOG_SECURITY.md` §6), so a fabricated one reopens the
        // preemption attack it exists to close.
        created_by: first_tag_value(event, "created_by")
            .unwrap_or_default()
            .to_string(),
