...
use super::bindings::CodeExecutionMode;
...
#[cfg(unix)]
const PINNED_GIT_HELPER_ARGUMENT: &str = "--schoolx-code-pinned-git-helper-v1";
#[cfg(unix)]
...
impl CodePinnedGitOperation {
    ...
    pub(crate) fn execute(self) -> Result<Vec<u8>, String> {
        ...
        let output = spawn_pinned_git_helper(&self.request, &self.directories)?;
        verify_pinned_target_chain(&self.request, &self.directories)?;
        Ok(output)
    }
}
...
pub fn prepare_execution_root(
    ...
    let descriptor = match input.execution_mode {
        CodeExecutionMode::Local => CodeWorktreeDescriptor {
        ...
        CodeExecutionMode::Worktree => {
            prepare_managed_worktree(&repository, &base_commit, nest_root)?
        }
    };

    let status = revalidate_execution_root(&descriptor, nest_root)?;
    Ok(CodeWorktreePrepareResult {
...
pub fn revalidate_execution_root(
    ...
    let execution_repository = discover_repository(&execution_root)?;
    ...
    let resolved_base = resolve_commit(&execution_root, &descriptor.base_ref)?;
    ...
    let head_commit = resolve_commit(&execution_root, "HEAD")?;
    let branch = repository_branch(&execution_root)?;
    let dirty = repository_is_dirty(&execution_root)?;
...
fn prepare_managed_worktree(
    ...
    add_managed_worktree(repository, &target, base_commit)?;
    ...
    checkout_managed_worktree(&execution_root, base_commit)?;
...
fn add_managed_worktree(
    ...
    run_git_in_pinned_directory(target, request).map(|_| ())
}
...
fn add_managed_worktree(
    ...
    let target = path_to_string(target, "managed worktree target")?;
    run_git(
...
#[cfg(unix)]
fn checkout_managed_worktree(execution_root: &Path, base_commit: &str) -> Result<(), String> {
    ...
    run_git_in_pinned_directory(execution_root, request).map(|_| ())
}
...
fn repository_is_dirty(repository_root: &Path) -> Result<bool, String> {
    let output = run_git(
        repository_root,
        &[
...
fn repository_branch(repository_root: &Path) -> Result<Option<String>, String> {
    let output = run_git(
        repository_root,
...
fn repository_identity(common_dir: &Path) -> Result<String, String> {
...
fn run_git(cwd: &Path, args: &[OsString], operation: GitOperation) -> Result<Vec<u8>, String> {
    let disabled_filter_keys = if operation.may_run_repository_filters() {
        repository_filter_overrides(cwd)?
    } else {
        Vec::new()
    };
    run_git_with_filter_overrides(cwd, args, operation, &disabled_filter_keys)
}

fn repository_filter_overrides(cwd: &Path) -> Result<Vec<String>, String> {
    let mut overrides = BTreeSet::new();
    let local_output = run_git_with_filter_overrides(
        cwd,
        &[
    ...
    if worktree_config_enabled {
        let worktree_output = run_git_with_filter_overrides(
            cwd,
            &[
                OsString::from("config"),
                OsString::from("--worktree"),
...
fn collect_local_filter_overrides(
    output: &[u8],
...
fn collect_filter_override_names(
    output: &[u8],
    overrides: &mut BTreeSet<String>,
) -> Result<(), String> {
...
fn run_git_with_filter_overrides(
    cwd: &Path,
    args: &[OsString],
...
fn run_git_in_pinned_directory(
    ...
    let operation = prepare_pinned_git_operation(directory_path, request)?;
...
/// Run the private pre-Tauri helper when this process was spawned with a
/// pinned target directory on stdin. Successful execution replaces this
/// process with Git and therefore does not return.
#[cfg(unix)]
pub(crate) fn run_pinned_git_helper_if_requested() -> Result<bool, String> {
    let mut arguments = std::env::args_os();
    let _program = arguments.next();
    let Some(argument) = arguments.next() else {
        return Ok(false);
    };
    if argument != PINNED_GIT_HELPER_ARGUMENT {
        return Ok(false);
    }
    if arguments.next().is_some() {
        return Err("pinned Git helper received unexpected arguments".to_string());
    }
    execute_pinned_git_helper()?;
    Ok(true)
}
...
fn pinned_git_command(request: &PinnedGitRequest) -> Result<Command, String> {
    let (git_executable, arguments, disabled_filter_keys) = match request {
        PinnedGitRequest::WorktreeAdd {
            git_executable,
            git_common_dir,
            base_commit,
            ...
            (
                validate_helper_git_executable(git_executable)?,
                vec![
    ...
    configure_git_environment(&mut command, GitOperation::Mutating, disabled_filter_keys);
...
fn spawn_pinned_git_helper(
    ...
    #[cfg(not(test))]
    command.arg(PINNED_GIT_HELPER_ARGUMENT);
    #[cfg(test)]
    command.args([
        "--exact",
    ...
    capture_child(child, "pinned git")
}
...
fn run_git_executable_with_filter_overrides(
    ...
    capture_child(child, "git")
}

fn capture_child(mut child: Child, label: &str) -> Result<Vec<u8>, String> {
    ...
    if !status.success() {
        ...
        return Err(if message.is_empty() {
            format!("{label} exited with status {status}{suffix}")
        } else {
...
mod tests {
    ...
    fn helper_uses_the_pinned_directory_if_the_name_changes_after_validation() -> Result<(), String>
    {
        ...
        spawn_pinned_git_helper(&operation.request, &operation.directories)?;
