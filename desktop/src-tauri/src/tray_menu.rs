...
//! projection here so the native menu can remain useful while the app is hidden.
...
/// Loads the generated SchoolX mark for use as a macOS template image.
///
/// Template rendering uses the PNG's alpha channel and lets macOS choose the
/// foreground color for light, dark, active, and inactive menu bars.
fn tray_schoolx_icon() -> tauri::Result<Image<'static>> {
...
/// Installs the persistent SchoolX tray icon with the initial empty activity menu.
...
pub fn take_tray_actions<R: Runtime>(app: AppHandle<R>) -> Result<Vec<TrayAction>, String> {
    ...
    let mut queue = state
        .action_queue
        .lock()
        .map_err(|_| format!("{PRODUCT_NAME} tray action queue is unavailable"))?;
...
pub fn update_tray_agent_activity<R: Runtime>(
    ...
    let mut activity_items = state
        .activity_items
        .lock()
        .map_err(|_| format!("{PRODUCT_NAME} tray menu state is unavailable"))?;
            ...
            })
    {
        ...
        let tray = app
            .tray_by_id(TRAY_ID)
            .ok_or_else(|| format!("{PRODUCT_NAME} tray icon is not available"))?;
