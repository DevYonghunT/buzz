...
#[cfg(test)]
mod tests {
    // Fixture nests use the real production nest name via
    // `product::NEST_DIR_PROD` rather than a hardcoded literal. These paths are
    // built directly instead of going through `nest_dir()`, so a literal here
    // silently drifted from the product layer — it still said `.buzz` after the
    // rebrand, which read as un-rebranded production code to anyone grepping.
    ...
    fn ensure_repos_symlink_none_creates_real_dir() {
        ...
        let root = tmp.path().join(crate::product::NEST_DIR_PROD);
    ...
    fn validate_repos_dir_rejects_nest_ancestor() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("home").join(crate::product::NEST_DIR_PROD);
        fs::create_dir_all(&root).unwrap();
