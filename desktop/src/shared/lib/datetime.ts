...
const WEEKDAY_MONTH_DAY_OPTIONS: Intl.DateTimeFormatOptions = {
  weekday: "long",
  month: "long",
  day: "numeric",
};
...
const SHORT_WEEKDAY_SHORT_MONTH_DAY_OPTIONS: Intl.DateTimeFormatOptions = {
  weekday: "short",
  month: "short",
  day: "numeric",
};

const SHORT_MONTH_DAY_YEAR_OPTIONS: Intl.DateTimeFormatOptions = {
  month: "short",
  day: "numeric",
  year: "numeric",
};
...
export type FormatDayGroupLabelOptions = {
  locale?: AppLocale;
  nowSeconds?: number;
};

export type FormatItemTimestampOptions = {
  locale?: AppLocale;
  nowSeconds?: number;
  withTime?: boolean;
};
...
function relativeDayLabel(
  locale: AppLocale,
  key: "time.today" | "time.yesterday",
): string {
  return i18n.getFixedT(locale)(key);
}

function joinDayAndTime(
  dayLabel: string,
  time: string,
  locale: AppLocale,
): string {
  return locale === "ko" ? `${dayLabel} ${time}` : `${dayLabel} at ${time}`;
}
 ...
 *
 * `locale` defaults to the app fallback for compatibility. UI callers should
 * pass the active app locale. `nowSeconds` is injectable so the relative bands
 * are testable; it must stay an argument rather than a captured constant,
 * because a label rendered before midnight has to say "Yesterday" once the day
 * rolls over.
 */
export function formatDayGroupLabel(
  unixSeconds: number,
  options?: FormatDayGroupLabelOptions,
): string;
...
export function formatDayGroupLabel(
  unixSeconds: number,
  options: FormatDayGroupLabelOptions | number = {},
): string {
  const normalizedOptions: FormatDayGroupLabelOptions =
  ...
  if (dayDiff === 0) return relativeDayLabel(locale, "time.today");
  if (dayDiff === 1) return relativeDayLabel(locale, "time.yesterday");
  ...
  if (dayDiff > 1 && dayDiff < WEEKDAY_BAND_DAYS) {
    return formatDateTime(date, locale, WEEKDAY_OPTIONS);
  }

  return date.getFullYear() === now.getFullYear()
    ? formatDateTime(date, locale, WEEKDAY_MONTH_DAY_OPTIONS)
    : formatDateTime(date, locale, MONTH_DAY_YEAR_OPTIONS);
}
...
export function formatItemTimestamp(
  ...
  }: FormatItemTimestampOptions = {},
): string {
  const date = new Date(unixSeconds * 1_000);
  const now = new Date(nowSeconds * 1_000);
  const dayDiff = calendarDaysBetween(now, date);
  const time = formatDateTime(date, locale, TIME_OPTIONS);

  if (dayDiff === 0) return time;

  let dayLabel: string;
  if (dayDiff === 1) {
    dayLabel = relativeDayLabel(locale, "time.yesterday");
  } else if (dayDiff > 1 && dayDiff < WEEKDAY_BAND_DAYS) {
    dayLabel = formatDateTime(date, locale, WEEKDAY_OPTIONS);
  } else {
    dayLabel =
      date.getFullYear() === now.getFullYear()
        ? formatDateTime(date, locale, SHORT_WEEKDAY_SHORT_MONTH_DAY_OPTIONS)
        : formatDateTime(date, locale, SHORT_MONTH_DAY_YEAR_OPTIONS);
  }

  return withTime ? joinDayAndTime(dayLabel, time, locale) : dayLabel;
}
