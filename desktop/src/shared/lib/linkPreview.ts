...
import { DEEP_LINK_SCHEME, LEGACY_DEEP_LINK_SCHEME } from "../product/index.ts";
...
const ENTITY_LINK_SCHEMES_RE = `(?:${DEEP_LINK_SCHEME}|${LEGACY_DEEP_LINK_SCHEME})`;
const SUPPORTED_URL_RE = new RegExp(
  String.raw`(^|[\s([{<>"'])(https:\/\/[^\s<>"'\]]+|https?:\/\/[^\s<>"'\]]+\/git\/[a-f0-9]{64}\/[^\s<>"'\]]+|${ENTITY_LINK_SCHEMES_RE}:\/\/(?:pr|issue|repo|project)\?[^\s<>"'\]]+|(?:(?:www\.)?github\.com|(?:www\.)?linear\.app|drive\.google\.com|docs\.google\.com)\/[^\s<>"'\]]+)`,
  "gi",
);
const MARKDOWN_SUPPORTED_LINK_RE = new RegExp(
  String.raw`!?\[([^\]\n]+)\]\((https:\/\/[^)\s<>"']+|https?:\/\/[^)\s<>"']+\/git\/[a-f0-9]{64}\/[^)\s<>"']+|${ENTITY_LINK_SCHEMES_RE}:\/\/(?:pr|issue|repo|project)\?[^)\s<>"']+|(?:(?:www\.)?github\.com|(?:www\.)?linear\.app|drive\.google\.com|docs\.google\.com)\/[^)\s<>"']+)\)`,
  "gi",
);
...
/**
 * Map a current or legacy entity deep link onto a preview card. The
 ...
 * is normalized to the canonical current-scheme repository deep link: the raw
 * git transport endpoint is not a browsable page, and the native href gives the
...
export function extractSupportedLinkPreviews(
  ...
  for (const candidate of candidates) {
    ...
    // current-scheme repository href. External web links continue through the
    // snapshot path.
    if (!preview || isEntityLink(preview.href) || seen.has(preview.href))
      continue;
