/**
 * SchoolX deep links for relay-hosted git entities, mirroring
 * `features/messages/lib/messageLink.ts` for message links.
 *
 * Formats:
 *   schoolx://repo?owner=<owner-pubkey>&d=<repo-dtag>[&tab=<tab>][&commit=<git-hash>]
 *   schoolx://project?owner=<owner-pubkey>&d=<project-dtag>[&tab=<tab>]
 *   schoolx://pr?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *   schoolx://issue?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *
 ...
 * on both sides).
 *
 * Links written before the SchoolX rename remain readable through the legacy
 * product scheme, while every builder emits only the current scheme.
 */
...
const READABLE_ENTITY_LINK_SCHEMES: readonly string[] = [
  DEEP_LINK_PROTOCOL,
  LEGACY_DEEP_LINK_PROTOCOL,
];
const READABLE_ENTITY_LINK_PREFIXES: readonly string[] = [
  DEEP_LINK_URL_PREFIX,
  LEGACY_DEEP_LINK_URL_PREFIX,
];
const ENTITY_LINK_HOSTS = ["pr", "issue", "repo", "project"] as const;
...
/** Build a current-scheme repository link (kind 30617). */
export function buildRepoLink(input: {
  ...
  return `${DEEP_LINK_URL_PREFIX}repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildCommitLink(input: {
  ...
  return `${DEEP_LINK_URL_PREFIX}repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}&tab=commits&commit=${input.commitHash.toLowerCase()}`;
}

/** Build a current-scheme project link (kind 30621). */
export function buildProjectLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `${DEEP_LINK_URL_PREFIX}project?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}

/** Build a current-scheme pull-request link (kind 1618). */
export function buildPullRequestLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `${DEEP_LINK_URL_PREFIX}pr?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}

/** Build a current-scheme issue link (kind 1621). */
export function buildIssueLink(input: {
  ...
  checkEventId(input.id);
  checkCoordinate(input.owner, input.dtag);
  return `${DEEP_LINK_URL_PREFIX}issue?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}

/**
 * Cheap pre-check used by the markdown renderer and preview extraction
 * before parsing. Message links are intentionally excluded — they have their
 * own pill rendering path. Both current and legacy entity links are readable.
 */
export function isEntityLink(href: string | undefined | null): boolean {
  if (!href) return false;
  return READABLE_ENTITY_LINK_PREFIXES.some((prefix) =>
    ENTITY_LINK_HOSTS.some((host) => href.startsWith(`${prefix}${host}?`)),
  );
}

/**
 * Parse a current or legacy entity URL. Returns a discriminated result so
...
export function parseEntityLink(url: string): EntityLinkParseResult {
  ...
  if (!READABLE_ENTITY_LINK_SCHEMES.includes(parsed.protocol)) {
