...
test("commit links select an exact repository commit", () => {
  ...
  assert.equal(
    link,
    `schoolx://repo?owner=${OWNER}&d=buzz-world&tab=commits&commit=${EVENT_ID}`,
  );
  assert.deepEqual(parseEntityLink(link), {
    ok: true,
    value: {
...
test("parseEntityLink preserves legacy scheme read compatibility", () => {
  assert.deepEqual(
    parseEntityLink(`buzz://pr?id=${EVENT_ID}&owner=${OWNER}&d=buzz-world`),
    {
      ok: true,
      value: { type: "pr", id: EVENT_ID, owner: OWNER, dtag: "buzz-world" },
    },
...
test("isEntityLink matches entity hosts and excludes message links", () => {
  assert.equal(isEntityLink(`schoolx://pr?id=${EVENT_ID}`), true);
  assert.equal(isEntityLink(`schoolx://repo?owner=${OWNER}`), true);
...
test("coordinate links carry an optional workspace tab", () => {
  ...
  assert.equal(link, `schoolx://project?owner=${OWNER}&d=buzz-world&tab=prs`);
  assert.deepEqual(parseEntityLink(link), {
    ok: true,
