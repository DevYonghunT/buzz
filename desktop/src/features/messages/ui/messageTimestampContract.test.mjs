...
test("a message header shows the relative label, including the time", () => {
  ...
  assert.match(
    source,
    /: formatItemTimestamp\(createdAt, \{ locale, withTime: true \}\)/,
  );
});
...
test("the continuation gutter stays clock-only", () => {
  ...
  assert.match(
    source,
    /hideDayPeriod \? formatTimeWithoutDayPeriod\(formatTime\(createdAt, locale\), locale\)/,
  );
});
...
test("the tooltip still carries the unabbreviated timestamp", () => {
  assert.match(source, /formatFullDateTime\(createdAt, locale\)/);
});
