...
import { DEEP_LINK_URL_PREFIX } from "../../../shared/product/index.ts";
...
const CANONICAL_HREF = `${DEEP_LINK_URL_PREFIX}message?channel=${CHANNEL_ID}&id=${MESSAGE_ID}`;
...
const CANONICAL_REPO_HREF = `${DEEP_LINK_URL_PREFIX}repo?owner=${OWNER}&d=buzz-world`;
const ISSUE_ID = "b".repeat(64);
const ISSUE_HREF = `buzz://issue?id=${ISSUE_ID}&owner=${OWNER}&d=buzz-world`;
const CANONICAL_ISSUE_HREF = `${DEEP_LINK_URL_PREFIX}issue?id=${ISSUE_ID}&owner=${OWNER}&d=buzz-world`;
...
test("resolves a composer preview and canonicalizes the underlying href", () => {
  assert.deepEqual(
    ...
    { channelName: "general", href: CANONICAL_HREF },
  );
});
...
test("resolves channel and entity links as composer chips", () => {
  ...
  assert.deepEqual(
    ...
    {
      channelName: "general",
      href: `${DEEP_LINK_URL_PREFIX}message?channel=${CHANNEL_ID}&id=${CHANNEL_MESSAGE_ID}`,
    },
  );
  assert.deepEqual(
...
test("real markdown-it parsing materializes a restored message link", () => {
  ...
  assert.ok(
    html.includes(`data-href="${CANONICAL_HREF.replace("&", "&amp;")}"`),
  );
});
...
test("real markdown-it parsing materializes mixed Buzz permalink chips", () => {
  ...
  assert.match(
    html,
    /data-href="schoolx:\/\/repo\?owner=a{64}&amp;d=buzz-world/,
  );
});
...
test("real markdown-it parsing preserves underscores in restored entity links", () => {
  ...
  assert.match(html, /data-href="schoolx:\/\/repo\?owner=a{64}&amp;d=my_repo"/);
