...
import { resolveProjectRelayBase } from "./projectRelayBase";
import type {
  ProjectPullRequest,
...
export async function fetchProjects(): Promise<Project[]> {
  const [events, deletionEvents, relayBase] = await Promise.all([
    ...
    resolveProjectRelayBase(),
  ]);

  return dedup(events)
    .map((event) => eventToProject(event, relayBase))
    .filter(
      (project) =>
...
async function fetchProject(projectId: string): Promise<Project | null> {
  ...
  const [events, relayBase] = await Promise.all([
    relayClient.fetchEvents({
      kinds: [KIND_REPO_ANNOUNCEMENT],
      ...(owner ? { authors: [owner] } : {}),
      "#d": [dtag],
  ...
  const project =
    deduped.length > 0 ? eventToProject(deduped[0], relayBase) : null;
  if (!project) {
    return null;
  }
