...
test("projectShareLink links explicit projects by their 30621 coordinate", () => {
  assert.equal(
    ...
    `${DEEP_LINK_URL_PREFIX}project?owner=${OWNER}&d=pollinator`,
  );
});
...
test("projectShareLink carries the active workspace tab for both link kinds", () => {
  assert.equal(
    ...
    `${DEEP_LINK_URL_PREFIX}project?owner=${OWNER}&d=pollinator&tab=prs`,
  );
  // Legacy projects share as repository links and keep the tab too.
  assert.equal(
    projectShareLink({ projectAddress: REPO_ADDRESS }, "issues"),
    `${DEEP_LINK_URL_PREFIX}repo?owner=${OWNER}&d=flappy-bee&tab=issues`,
  );
});
...
test("projectShareLink links legacy projects as their backing repository", () => {
  assert.equal(
    ...
    `${DEEP_LINK_URL_PREFIX}repo?owner=${OWNER}&d=flappy-bee`,
  );
});
...
test("issue and pull request links carry the event id and repo coordinate", () => {
  assert.equal(
    ...
    `${DEEP_LINK_URL_PREFIX}issue?id=${EVENT_ID}&owner=${OWNER}&d=flappy-bee`,
  );
  assert.equal(
    pullRequestShareLink({ id: EVENT_ID, repoAddress: REPO_ADDRESS }),
    `${DEEP_LINK_URL_PREFIX}pr?id=${EVENT_ID}&owner=${OWNER}&d=flappy-bee`,
  );
});
