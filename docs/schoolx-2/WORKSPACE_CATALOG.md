...
> **구현 상태 — 세션 D3(2026-08-04)에서 닫혔다.** 바로 위 문단의 마지막 문장
> — "다시 만들기"를 선택하면 `generation`을 올려 새 ID로 만든다 — 이 이제
> 실제로 그렇게 한다. 설정 카드의 `user_action` 안내에 버튼이 붙고
> (`catalog-recreate-<item_key>`), 적용 인자가 `Vec<String>`에서
> `Vec<Selection>`이 되어 항목마다 `recreate_from`을 실어 보낸다.
>
> **세대는 preflight가 알아내지 않는다. 사용자 확인이 나른다.** 탄 세대를
> 조회할 방법이 없기 때문이다 — 바로 위 1–3번이 그 이유다(삭제된 채널의
> 증명서는 읽히지 않는다). 그래서 화면이 **보여준** 세대를 그대로 되돌려
> 보내고, saga는 preflight가 지금 보고하는 세대가 그 값과 같을 때만 한 칸
> 올린다. 그 일치 검사가 낡은 화면·두 번째 클릭·이미 처리한 다른 관리자를
> 전부 no-op으로 만든다 — 재생성에 별도 멱등성 장치가 없는 이유다.
>
> **한 번에 한 칸만 올린다.** `g + 1`도 타 있으면 그 실행이 `g + 1`을 다시
> 보고하고 사용자가 다시 누른다. 빈 세대를 탐색하지 않는 이유는 그것이
> 사용자가 승인한 것보다 멀리 가기 때문이다.
>
> 새 세대는 **새 방**이므로 이전 세대에서 읽어 온 단계 상태를 버린다. 이
> 초기화가 없으면 `resume`이 `not_owned`로 끝난 항목 — 단계에 **다른 사람의**
> 진행이 적혀 있다 — 을 재생성할 때 saga가 채널 생성을 건너뛰고 있지도 않은
> 방에 그다음 단계를 건다. `recreating_a_partially_applied_item_starts_the_new_room_from_scratch`가
> 그 경로를 고정한다.
>
> 설계와 남는 한계는 [`CATALOG_RECREATE.md`](CATALOG_RECREATE.md). 요약하면
> 이것은 **선점을 막지 않는다** — 공격자가 다음 세대들을 미리 차지할 수
> 있으므로, 영구 차단이 유한한 경합으로 바뀐 것이다.
