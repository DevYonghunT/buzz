...
> 우선한다. 배포 아이콘과 webview 내부 SchoolX 시각 체계의 현재 경계 및 다음
> 구현 slice는
...
### 세션 G — 자체 호스팅 릴레이 온보딩 · **완료 (2026-08-05)**

학교가 자기 릴레이로 앱에 들어오는 문을 냈다. 설계는
[`SELF_HOSTED_ONBOARDING.md`](SELF_HOSTED_ONBOARDING.md), 계획은
[`plans/2026-08-05-self-hosted-onboarding.md`](plans/2026-08-05-self-hosted-onboarding.md).

세션 G에서 확인된 사실 중 다음 셋은 이후 세션이 전제해야 한다.

1. **연결 경로는 원래 있었다. 없던 것은 문이다.** `InviteRedeemForm`의
   `canSubmit`이 `normalizedRelayUrl`만으로 참이고 `startConnection`이 그대로
   연결한다. 이 세션이 만든 것은 그 경로로 가는 진입점과 문구뿐이다 —
   비슷한 「기능이 없다」는 보고를 받으면 **정말 없는지 먼저 확인한다.**
2. **다이얼로그 밖에 띄운 것은 보이지 않는다.** 호스팅 다이얼로그 옆에
   `fixed`로 배치한 링크는 오버레이 블러에 묻히고 클릭도 가로채였다.
   `toBeVisible()`은 통과하고 `click()`이 타임아웃 나는 모양이라, 존재
   단언만으로는 잡히지 않는다. 다이얼로그 위에 무언가를 얹어야 하면 그
   다이얼로그의 **자식**이어야 한다.
3. **upstream 파일 둘을 건드린다.** `WelcomeSetup.tsx`와
   `HostedCommunityOnboarding.tsx`는 SchoolX가 처음 손댄 upstream 원본이고
   upstream이 계속 고친다(#2738, #2862). 변경은 각각 렌더 한 줄과 prop
   하나로 묶었고 나머지는 전부 `SelfHostedRelayEntry.tsx`에 있다. **동기화
   때 이 두 파일의 충돌을 먼저 본다.**

**세션 G에서 넘긴 것.**

1. **사람 검증이 남았다.** 「소스를 보지 않은 사람이 화면 안내만으로
   들어간다」가 이 작업의 완료 기준인데, 앱이 이미 커뮤니티에 붙어 있어
   `community.needsSetup`이 거짓이라 온보딩 화면에 닿지 못했다. Playwright가
   그 화면을 띄워 스크린샷까지 냈으나 그건 존재 확인이지 사용성 확인이 아니다.
   새 기기나 초기화된 상태에서 한 번 밟아 봐야 한다.
2. **한국어 로케일 확인.** e2e 하네스가 영어라 스펙 스크린샷도 영어다. 실제
   앱에서 이 화면이 `ko`를 타는지는 확인하지 않았다.
3. **연결 실패 설명.** 오타·미기동·방화벽을 구별하는 것은 설계 §4.4에
   적었으나 이번 범위에 넣지 않았다.
4. **모바일.** 별도다.
