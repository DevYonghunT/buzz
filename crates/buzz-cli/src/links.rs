//! Canonical `schoolx://` deep links for SchoolX entities.
//!
//! SchoolX Desktop renders these links as rich preview cards in chat and
...
//!
//! Message parsing remains compatible with legacy `buzz://` links already
//! stored in shared community history; builders emit only the current scheme.
//!
...
pub(crate) const DEEP_LINK_SCHEME: &str = "schoolx";
pub(crate) const DEEP_LINK_URL_PREFIX: &str = "schoolx://";
const LEGACY_DEEP_LINK_SCHEME: &str = "buzz";

/// A validated current or legacy message deep link.
...
/// Parse a `schoolx://message?channel=<uuid>&id=<event>[&thread=<root>]` link.
///
...
pub fn parse_message_link(input: &str) -> Result<MessageLink, CliError> {
    let url = url::Url::parse(input.trim())
        .map_err(|_| CliError::Usage("invalid SchoolX message link".into()))?;
        ...
        || url.fragment().is_some()
    {
        return Err(CliError::Usage(format!(
            "expected a {DEEP_LINK_URL_PREFIX}message or legacy \
                 {LEGACY_DEEP_LINK_SCHEME}://message link without credentials or a fragment"
        )));
    }
        ...
        let slot = match key.as_ref() {
            ...
            _ => {
                return Err(CliError::Usage(
                    "SchoolX message link contains an unsupported query parameter".into(),
                ))
            }
        };
        if slot.replace(value.into_owned()).is_some() {
            return Err(CliError::Usage(format!(
                "SchoolX message link contains more than one {key} parameter"
            )));
        }
    }
    ...
    let channel = channel
        ...
        .ok_or_else(|| CliError::Usage("SchoolX message link is missing channel".into()))?;
    let message = message
        .filter(|value| !value.is_empty())
        .ok_or_else(|| CliError::Usage("SchoolX message link is missing id".into()))?;
    if thread.as_deref() == Some("") {
        return Err(CliError::Usage(
            "SchoolX message link contains an empty thread parameter".into(),
        ));
    }

    let channel_id = uuid::Uuid::parse_str(&channel)
        .map_err(|_| {
            CliError::Usage("SchoolX message link contains an invalid channel UUID".into())
        })?
        .to_string();
...
fn canonical_event_id(value: &str, parameter: &str) -> Result<String, CliError> {
    if value.len() != 64 || !value.chars().all(|character| character.is_ascii_hexdigit()) {
        return Err(CliError::Usage(format!(
            "SchoolX message link contains an invalid {parameter} event ID"
        )));
    }
    Ok(value.to_ascii_lowercase())
}

/// Whether a d-tag can be expressed in a SchoolX entity link.
///
...
/// Build a `schoolx://repo` link for a repository announcement (kind 30617).
pub fn repo_link(owner: &str, repo_id: &str) -> String {
    format!("{DEEP_LINK_URL_PREFIX}repo?owner={owner}&d={repo_id}")
}

/// Build a `schoolx://project` link for a project announcement (kind 30621).
pub fn project_link(owner: &str, project_id: &str) -> String {
    format!("{DEEP_LINK_URL_PREFIX}project?owner={owner}&d={project_id}")
}

/// Build a `schoolx://pr` link for a pull request event (kind 1618).
pub fn pull_request_link(event_id: &str, owner: &str, repo_id: &str) -> String {
    format!("{DEEP_LINK_URL_PREFIX}pr?id={event_id}&owner={owner}&d={repo_id}")
}

/// Build a `schoolx://issue` link for an issue event (kind 1621).
pub fn issue_link(event_id: &str, owner: &str, repo_id: &str) -> String {
    format!("{DEEP_LINK_URL_PREFIX}issue?id={event_id}&owner={owner}&d={repo_id}")
}
...
mod tests {
    ...
    fn parses_message_link_with_thread_root() {
        let parsed = parse_message_link(&format!(
            "{DEEP_LINK_URL_PREFIX}message?channel={CHANNEL}&id={MESSAGE}&thread={THREAD}"
        ))
        .unwrap();
    ...
    fn normalizes_message_link_identifiers() {
        let parsed = parse_message_link(&format!(
            "{DEEP_LINK_URL_PREFIX}message?channel={}&id={}",
    ...
    fn rejects_message_link_that_could_change_connection_context() {
        for link in [
            format!("{DEEP_LINK_URL_PREFIX}message?channel={CHANNEL}&id={MESSAGE}&relay=other"),
            format!("{DEEP_LINK_SCHEME}://user:secret@message?channel={CHANNEL}&id={MESSAGE}"),
            format!("{DEEP_LINK_URL_PREFIX}message?channel={CHANNEL}&id={MESSAGE}#fragment"),
        ] {
    ...
    fn rejects_duplicate_or_malformed_message_link_identifiers() {
        for link in [
            format!(
                "{DEEP_LINK_URL_PREFIX}message?channel={CHANNEL}&channel={CHANNEL}&id={MESSAGE}"
            ),
            format!("{DEEP_LINK_URL_PREFIX}message?channel=not-a-uuid&id={MESSAGE}"),
            format!("{DEEP_LINK_URL_PREFIX}message?channel={CHANNEL}&id=not-an-event"),
            format!("{DEEP_LINK_URL_PREFIX}message?channel={CHANNEL}&id={MESSAGE}&thread="),
        ] {
            assert!(parse_message_link(&link).is_err(), "accepted {link}");
        }
    }
