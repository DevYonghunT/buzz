...
pub enum MessagesCmd {
    ...
    #[command(
        after_help = "Examples:\n  buzz messages thread --channel <UUID> --event <EVENT_ID>\n  buzz messages thread --link 'schoolx://message?channel=<UUID>&id=<EVENT_ID>&thread=<ROOT_ID>'"
    )]
    Thread {
        ...
        #[arg(long, required_unless_present = "link", conflicts_with = "link")]
        event: Option<String>,
        /// Canonical schoolx://message deep link; legacy Buzz links are also accepted
...
mod tests {
    ...
    fn messages_thread_accepts_link_or_explicit_identifiers() {
        ...
        let link = format!(
            "{}message?channel={channel}&id={event}",
            crate::links::DEEP_LINK_URL_PREFIX
        );
