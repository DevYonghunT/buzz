    ...
    for channel in &channels {
        ...
        if target_channel.is_none() {
            // kind:39000 — channel metadata
            {
                ...
                // Must match `emit_group_discovery_events` in the relay —
                // clients read this as immutable channel provenance and cannot
                // tell which producer wrote the event. Omitting it here would
                // make a backfilled channel look creatorless, which SchoolX
                // catalog adoption reads as "cannot verify" and refuses.
                tags.push(Tag::parse([
                    "created_by",
