...
use crate::{
    ...
    handlers::ingest::PrincipalClass,
    state::AppState,
};
...
async fn authorize_workflow_read(
    ...
    let principal_class = PrincipalClass::from_agent_owner(
        super::relay_members::resolve_agent_owner(state, &tenant, &pubkey, auth_tag)
            .await
            .map_err(|error| internal_error(&error))?,
    );
    let accessible = if principal_class.requires_explicit_channel_membership() {
        state
