...
if [[ "$instance_id" != "io.github.schoolx520.app.dev" && "$instance_id" != io.github.schoolx520.app.dev.* ]]; then
    echo "reset-desktop-standalone-state: refusing non-dev bundle identifier: $instance_id" >&2
    exit 1
fi
if [[ "$keyring_service" != "schoolx-desktop-dev" && "$keyring_service" != schoolx-desktop-dev.* ]]; then
