...
BUZZ_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${BUZZ_VITE_PORT} --strictPort\"},\"identifier\":\"io.github.schoolx520.app.dev\",\"productName\":\"SchoolX Dev\"}"
...
if git rev-parse --is-inside-work-tree &>/dev/null; then
    ...
    if [[ -n "$GIT_COMMON_DIR" && "$GIT_DIR" != "$GIT_COMMON_DIR" ]]; then
        ...
        if [[ "${BUZZ_SHARE_IDENTITY:-0}" == "1" ]]; then
            KEYRING_SERVICE="schoolx-desktop-dev"
            KEYRING_BLOB=""
            case "$(uname -s)" in
                Darwin)
            ...
            CANONICAL_KEY="$HOME/Library/Application Support/io.github.schoolx520.app.dev/identity.key"

            # No legacy fallback: upstream also reads the pre-rename
            # `xyz.block.sprout.app.dev` key here, but that identity belongs to
            # a Buzz install. Sharing it into a SchoolX worktree instance would
            # sign SchoolX events with a co-installed Buzz's key.
            ...
            if [[ -n "$SHARED_IDENTITY" ]]; then
                ...
                echo "⚠ BUZZ_SHARE_IDENTITY=1 but no identity found in keyring service $KEYRING_SERVICE or at $CANONICAL_KEY — run SchoolX from repo root first" >&2
            fi
        fi
        ...
        if swift "$GENERATE_DEV_ICON" "$BASE_ICON" "$DEV_ICON" "$BUZZ_WORKTREE_LABEL"; then
            ...
            BUZZ_TAURI_CONFIG="{\"build\":{\"devUrl\":\"${DEV_URL}\",\"beforeDevCommand\":\"exec ./node_modules/.bin/vite --port ${BUZZ_VITE_PORT} --strictPort\"},\"identifier\":\"io.github.schoolx520.app.dev.${BUZZ_INSTANCE_SLUG}\",\"productName\":\"SchoolX Dev (${BUZZ_WORKTREE_LABEL})\",\"bundle\":{\"icon\":[\"$DEV_ICON\"]}}"
        fi
    fi
fi
